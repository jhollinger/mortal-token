...
A token's == and === methods accept another token or a hash value. A return value of false means that the token/hash on the right is no longer (or never was) valid.
